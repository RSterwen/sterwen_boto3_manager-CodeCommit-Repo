...
def create_product(category, sku, **item):
        table = get_dynamo_table('products-sterwen')
        keys = {
            'category': category,
            'sku': sku,
        }
...
if __name__ == '__main__':
    ...
    dyn_create_table = create_dynamo_table(
            'products-sterwen',
            pk=[
            {
    ...
    create_dynamo_items('products-sterwen', items, keys=['category', 'sku'])
    ...
    items = query_products(Key('category').eq('apparel') & Key('sku').begins_with('foo'),
    filter_expr=Attr('in_stock').eq(True))
    x=len(items)
    print(x)
